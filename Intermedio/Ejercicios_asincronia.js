...
// 6. Crea una función getUsiario(id) que devuelva una promesa y simule una llamada a una API (que se demore 2s).
...
//    Usa async/await para llamar a getUsiario(id) y maneja los errores con try/catch.
function getUsiario(id) {
...
async function obtenerUsuario(id) {
    try {
        const usuario = await getUsiario(id)
        console.log(usuario)
    } catch (error) {
        console.log(error)
    }
...
function tareaUno() {
    return new Promise(resolver => {
        setTimeout(() => {
            console.log("Tarea uno completada")
            resolver()
        }, 1000)
    })
}
...
function tareaDos() {
    return new Promise(resolver => {
        setTimeout(() => {
            console.log("Tarea dos completada")
            resolver()
        }, 2000)
    })
}
...
function tareaTres() {
    return new Promise(resolver => {
        setTimeout(() => {
            console.log("Tarea tres completada")
            resolver()
        }, 1500)
    })
}
...
function esperarSegundos(segundos) {
    return new Promise(resolver => {
        setTimeout(resolver, segundos * 1000)
    })
}
...
function retirarDinero(cantidad) {
    return new Promise((resolver, rechazar) => {
        setTimeout(() => {
            if (saldo >= cantidad) {
                saldo -= cantidad
                resolver(`Operación exitosa, saldo restante: ${saldo}$`)
            } else {
                rechazar("Error: Fondos insuficientes")
            }
        }, 2000)
    })
}

async function cajeroAutomatico() {
    await revisarSaldo()
    
    console.log("Retirando 300$...")
    try {
        const resultado1 = await retirarDinero(300)
    ...
    console.log("Retirando 300$...")
    try {
        const resultado2 = await retirarDinero(300)
